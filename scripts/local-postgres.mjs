...
if (!['127.0.0.1', 'localhost'].includes(url.hostname))
  throw new Error('db:local only starts a loopback database.');
...
const pg = new EmbeddedPostgres({
  ...
  postgresFlags: ['-h', '127.0.0.1', '-k', '/private/tmp'],
  onLog: () => {},
...
await client.end();
console.log(
  `PostgreSQL ready on 127.0.0.1:${url.port}. Data persists in .local/postgres. Press Ctrl+C to stop.`,
);
let stopping = false;
